...
#   翻译成“真实的” RISC-V 汇编代码，分支/跳转不再降级为 nop，而是输出:
#     - add/sub/addi/lw/sw 均按模板生成
#     - beq/bne 生成 “beq xRs0,xRs1,loop”
#     - jal  生成 “jal x0,loop”
#     - jalr 生成 “jalr xRd,xRs1,imm” （如果确实有 jalr）
#   汇编文件末尾都加 “loop: j loop”
# ----------------------------------------
ASM_TPL = {
...
def make_asm(seq):
    ...
    for inst in seq:
        ...
        if op in ("add", "sub"):
            ...
            # 假设名字是 "jalr_rD_rS_imm" → rd, rs1, imm 三部分
            # 如果你的 dataset 没有 jalr，可以忽略这一分支
            rd = parts[1][1:]
            rs1 = parts[2][1:]
            imm = parts[3]
            ...
            # 其他未知操作，一律当成 nop（尽量 dataset 中不应该走到这里）
            lines.append("  nop")
...
#   1) 调用 riscv64-unknown-elf-as 汇编 .s → .o
#   2) 再用 riscv64-unknown-elf-objcopy 提取 .text 段的裸二进制
#   3) 截断/或 pad 到 N_INST*4 字节（每条指令 4 bytes）
#   所有超出部分截断，若不足则用 nop 填满。
#   返回值：一个 bytes 对象，长度正好 N_INST*4。
# ----------------------------------------
def assemble_and_pad(s_path: Path, bin_path: Path):
    ...
    # 3) pad：用 nop 的机器码 0x00000013 (addi x0,x0,0) 补齐
...
# ----------------------------------------
# main(): 依次输出 initial_instr.h、candidates.h、cand_feat.h
# ----------------------------------------
def main():
    ...
    # CSV 里 'sequence' 是一个 Python list 字符串，需要 eval() 还原
    # 例如 "['add_r1_r2_r3','beq_r12_r0',...]" → ["add_r1_r2_r3", "beq_r12_r0", ...]
    df['seq_list'] = df['sequence'].apply(eval)

    # 2) 生成 initial_instr.h —— 直接取第 0 条作为初始顺序
    init_seq = df.loc[0, 'seq_list']           # 第 0 条原始顺序
